/********************************************************
* Cdocs // index-aux.js - Javascript. // Archivo 'auxiliar' para index.js
* Versión 0.1.0
...
function buscarTipoNIF(letra){
	...
	/* La asignación siguiente: tipoNIF.I, es ficiticia (DNI). */ 
	...
	resultado = (tipoNIF[letra]) ? tipoNIF[letra] : "DESCONOCIDO";
	return resultado;
}
...
/**
* Función - documentarErrorAJAX - Listado de errores deconexión.
**/
function documentarErrorAJAX(textoError){
	...
	/*Añadidos, para aplicaciones*/
	"512":"Invalid XML format"
	};
	if (errorAJAX.hasOwnProperty(numError)) {
		return "Error " + numError + " : " + errorAJAX[numError];
	} else {
		if(isNaN(numError) === false){
			return "Error " + numError + " : Unknown";
		}else{
			if(textoError === ""){
